...
#которая нужна
    def findPos(self, img, N=0, draw=True): #h,w,c лучше сделать глобальными, чтобы не пресчитывать их каждый раз с учетом того что они константы
        res = []
        h, w, c = img.shape
        if self.results.multi_hand_landmarks:
            hand = self.results.multi_hand_landmarks[-1]  # tracking hand number N
            for id, lm in enumerate(hand.landmark):#в идеале прописать трекинг произвольного количества рук с учетом проверки на наличие на каждом шагу
